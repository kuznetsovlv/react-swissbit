 ...
 *
 * `next` and `previous` move through the list without wrapping around its
 * boundaries. Calling `next` at the last option or `previous` at the first
 * option leaves the current option unchanged.
 *
...
export function useOptions<T>(
    ...
    return useOptionsRaw({options, option: initialOption}, false);
}
